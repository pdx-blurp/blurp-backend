...
router.post("/get", function (req, res, next) {
	...
	client
		...
		.then((response) => {
			...
			const collection = database.collection("maps");
			collection
				.aggregate([{ $match: { mapID: mapID } }, { $unwind: "$groups" }, { $match: { "groups.groupID": groupID } }, { $project: { groups: 1, _id: 0 } }])
				.toArray()
				.then((output) => {
...
router.post("/create", function (req, res, next) {
	...
	const grpModel = ["groupName", "description"];
	if (!mapID) {
	...
	client
		...
		.then((response) => {
			...
			collection
				.updateOne({ mapID: mapID }, { $push: { groups: groupData } })
				.then((output) => {
...
router.patch("/update", function (req, res, next) {
	...
	client.connect().then((response) => {
		...
		collection
			.updateOne({ mapID: mapID, "groups.groupID": groupID }, { $set: updates }, { arrayFilters: [{ "grp.groupID": groupID }] })
			.then((output) => {
				if (output.modifiedCount === 1 && output.matchedCount === 1) {
...
router.delete("/delete", function (req, res, next) {
	...
	client
		...
		.then((response) => {
			...
			const collection = database.collection("maps");
			collection
				.updateOne({ mapID: mapID, "groups.groupID": groupID }, { $pull: { groups: { groupID: groupID }, "nodes.$[].groups": groupID } })
				.then((output) => {
					if (output.modifiedCount === 1 && output.matchedCount === 1) {
